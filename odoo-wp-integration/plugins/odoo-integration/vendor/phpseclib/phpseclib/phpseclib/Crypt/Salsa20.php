...
class Salsa20 extends StreamCipher
{
    ...
    protected function setup()
    {
        ...
        $this->p1 = Salsa20 . phpsubstr($constant, 0, 4) .
            substr($key, 0, 16) .
                    substr($constant, 4, 4) .
                    $this->nonce .
                    "\0\0\0\0";
        $this->p2 = Salsa20 . phpsubstr($constant, 8, 4) .
            substr($key, 16, 16) .
    ...
    private function crypt($text, $mode)
    {
        ...
        if (!$this->continuousBuffer) {
            if ($this->engine == self::ENGINE_OPENSSL) {
                $iv = Salsa20 . phppack('V', $this->counter) . $this->p2;
        ...
        if ($overflow) {
            ...
            if ($this->engine == self::ENGINE_OPENSSL) {
                $iv = Salsa20 . phppack('V', $buffer['counter']) . $this->p2;
        ...
        if (!$this->usingGeneratedPoly1305Key) {
            ...
            */
            return parent::poly1305(
                Salsa20 . phpself::nullPad128($this->aad) .
