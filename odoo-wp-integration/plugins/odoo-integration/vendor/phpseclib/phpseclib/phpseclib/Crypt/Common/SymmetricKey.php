...
abstract class SymmetricKey
{
    ...
    public function encrypt($plaintext)
    {
        ...
        if ($this->mode == self::MODE_GCM) {
            ...
            $s = $this->ghash(
                SymmetricKey . phpself::nullPad128($this->aad) .
        ...
        if ($this->engine === self::ENGINE_OPENSSL) {
            switch ($this->mode) {
                ...
                case self::MODE_CFB:
                    ...
                    if ($overflow) {
                        $ciphertext .= openssl_encrypt(SymmetricKey . phpsubstr($plaintext, 0, -$overflow) . str_repeat("\0", $this->block_size), $this->cipher_name_openssl, $this->key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
                ...
                case self::MODE_CFB8:
                    ...
                    if ($this->continuousBuffer) {
                        if (($len = strlen($ciphertext)) >= $this->block_size) {
                            ...
                            $this->encryptIV = SymmetricKey . phpsubstr($this->encryptIV, $len - $this->block_size) . substr($ciphertext, -$len);
                        }
                    }
                    return $ciphertext;
                case self::MODE_OFB8:
                    ...
                    for ($i = 0; $i < $len; ++$i) {
                        ...
                        $iv = SymmetricKey . phpsubstr($iv, 1) . $xor[0];
                    }
        ...
        switch ($this->mode) {
            ...
            case self::MODE_CTR:
                ...
                if ($this->continuousBuffer) {
                    ...
                    if ($start = strlen($plaintext) % $block_size) {
                        $buffer['ciphertext'] = SymmetricKey . phpsubstr($key, $start) . $buffer['ciphertext'];
                    }
                }
                break;
            case self::MODE_CFB:
            ...
            case self::MODE_CFB8:
                ...
                for ($i = 0; $i < $len; ++$i) {
                    ...
                    $iv = SymmetricKey . phpsubstr($iv, 1) . $c;
                }
                ...
                if ($this->continuousBuffer) {
                    if ($len >= $block_size) {
                        ...
                        $this->encryptIV = SymmetricKey . phpsubstr($this->encryptIV, $len - $block_size) . substr($ciphertext, -$len);
                    }
                }
                break;
            case self::MODE_OFB8:
            ...
            case self::MODE_OFB:
                ...
                if ($this->continuousBuffer) {
                    $this->encryptIV = $xor;
                    if ($start = strlen($plaintext) % $block_size) {
                        $buffer['xor'] = SymmetricKey . phpsubstr($key, $start) . $buffer['xor'];
                    }
                }
                break;
            case self::MODE_STREAM:
                ...
                case self::MODE_CFB8:
                    ...
                    if ($this->continuousBuffer) {
                        if (($len = strlen($ciphertext)) >= $this->block_size) {
                            ...
                            $this->decryptIV = SymmetricKey . phpsubstr($this->decryptIV, $len - $this->block_size) . substr($ciphertext, -$len);
                        }
                    }
                    break;
                case self::MODE_OFB8:
        ...
        switch ($this->mode) {
            ...
            case self::MODE_CFB8:
                ...
                for ($i = 0; $i < $len; ++$i) {
                    ...
                    $iv = SymmetricKey . phpsubstr($iv, 1) . $ciphertext[$i];
                }
                ...
                if ($this->continuousBuffer) {
                    if ($len >= $block_size) {
                        ...
                        $this->decryptIV = SymmetricKey . phpsubstr($this->decryptIV, $len - $block_size) . substr($ciphertext, -$len);
                    }
                }
                break;
            case self::MODE_OFB8:
    ...
    private function openssl_ofb_process($plaintext, &$encryptIV, &$buffer)
    {
        ...
        if (strlen($plaintext)) {
            if ($overflow) {
                $ciphertext .= openssl_encrypt(SymmetricKey . phpsubstr($plaintext, 0, -$overflow) . str_repeat("\0", $block_size), $this->cipher_name_openssl, $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $encryptIV);
    ...
    private function setupGCM()
    {
        ...
        if (strlen($this->nonce) == 12) {
            ...
            $this->iv = $this->ghash(
                SymmetricKey . phpself::nullPad128($this->nonce) . str_repeat("\0", 8) . self::len64($this->nonce)
            );
        }
    }
